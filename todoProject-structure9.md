...
- "새로고침해도 반영 안 됨" 현상의 정확한 원인 규명(코드 미반영 vs 토큰 만료 vs 다른 원인) — 진행 중
- 원인이 토큰 만료로 확인되면 `OAuth2AuthorizedClientManager` 기반 자동 갱신 로직 추가 필요
- 캘린더 관련 단위 테스트 코드는 아직 작성 전
- list.html/네비게이션에 `/calendar` 진입 링크 확인 필요